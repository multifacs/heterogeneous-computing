...
Result withAccessor(const std::vector<float> &A,
...

  {
    do {
      ...
      sycl::event event = queue.submit([&](sycl::handler &h) {
        ...
        auto aHandle =
            aBuffer.get_access<sycl::access::mode::read,
                               sycl::access::target::constant_buffer>(h);
        auto bHandle =
            bBuffer.get_access<sycl::access::mode::read,
                               sycl::access::target::constant_buffer>(h);
...
Result withSharedMemory(const std::vector<float> &A,
                                     const std::vector<float> &b,
                                     int iterationsLimit, float accuracyTarget,
                                     sycl::queue &queue) {
  Result result;
...
Result withDeviceMemory(const std::vector<float> &A,
                                     const std::vector<float> &b,
                                     int iterationsLimit, float accuracyTarget,
                                     sycl::queue &queue) {
  Result result;
