...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    sycl::event event = queue.submit([&](sycl::handler &h) {
      ...
      h.parallel_for(
          sycl::nd_range<2>(sycl::range<2>(stepsCount, stepsCount),
                            sycl::range<2>(groupSize, groupSize)),
          [=](sycl::nd_item<2> item) {
            float x = dx * (item.get_global_id(0) + 0.5);
            float y = dy * (item.get_global_id(1) + 0.5);
            float value = sycl::sin(x) * sycl::cos(y);
            float sum = sycl::reduce_over_group(item.get_group(), value,
                                                std::plus<float>());
            if (item.get_local_id(0) == 0 && item.get_local_id(1) == 0) {
              buffer[item.get_group(0) * item.get_group_range(0) +
                     item.get_group(1)] = sum;
            }
          });
    });
    queue.wait();
  ...
  std::cout << "  Elapsed time - " << (end - start) / 1e+6 << " ms"
            << std::endl;
  std::cout << "  Expected val - " << expected << std::endl;
  std::cout << "  Computed val - " << computed << std::endl;
