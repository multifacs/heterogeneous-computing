...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        sycl::event event = queue.submit([&](sycl::handler &h) {
            ...
            auto bufferHANDLER = RESULTBuffer.get_access<sycl::access::mode::write>(h);
            h.parallel_for(sycl::nd_range<2>(

                               sycl::range<2>(stepsCount, stepsCount), sycl::range<2>(groupSize, groupSize)),
                           ...
                           [=](sycl::nd_item<2> item) {
                               ...
                               if (item.get_local_id(0) == 0 && item.get_local_id(1) == 0) {
                                   bufferHANDLER[item.get_group(0) * item.get_group_range(0) + item.get_group(1)] = sum;
                               }
                           });
        });
        queue.wait();
